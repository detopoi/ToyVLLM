...
Chunked 模式同时启用 RECOMPUTE 抢占。物理 Block 不足时，Scheduler 优先把较新的
Prefill 请求移回 WAITING；必要时也可抢占 Decode 请求。它的已生成 token 和采样随机流
会保留，只释放 KV Cache，重新进入后从 Prompt 开头重算。`--show-schedule` 会额外打印
`preempted=[request_id]`，每条结果也会显示累计抢占次数。
...
普通分页推理通过 `--enable-prefix-cache` 开启，且必须同时设置
`--max-num-batched-tokens` 启用 Chunked Prefill。
